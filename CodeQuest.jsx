    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white p-4 md:p-8 font-sans">
        <header className="flex justify-between items-center mb-8 max-w-5xl mx-auto">
          <h2 className="text-3xl font-bold flex items-center gap-3"><Map /> Carte des Systèmes</h2>
          <Button variant="ghost" onClick={() => setView('menu')}>Retour</Button>
        </header>
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {MODULES.map((mod) => (
            <div key={mod.id} onClick={() => unlockedModules.includes(mod.id) && (setActiveModule(mod.id), setView('level'))}
              className={`relative p-6 rounded-2xl border-2 transition-all cursor-pointer group ${unlockedModules.includes(mod.id) ? 'bg-slate-800 border-slate-700 hover:border-indigo-500' : 'bg-slate-900 border-slate-800 opacity-60 grayscale'}`}>
